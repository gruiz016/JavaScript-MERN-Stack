(function() {
  ...
  var basket = {
    ...
    productAdd: function(el, data) {
      ...
      if (dataObject.category === 'arrangement') {
        ...
        newItem = new Arrangements(dataObject.itemname, dataObject.vasetype, dataObject.qty);
        ...
        for (item in dataObject) {
          ...
          if(RegExp('qty.+').test(item) && dataObject[item] > 0) {
            ...
            dataObject['color' + stemType] !== '---') {
              ...
              newItem.flowers.addStem(key, dataObject[item], dataObject['color' + stemName]);
            } else {
  ...
  function Arrangements(name, vase, quantity = 1) {
    this.type = 'floral';
    this.storage = 'cool';
    this.name = name;
    this.vase = vase;
