...
const listWidgets = (function () {
  ...
  return {
    ...
    apiKey: "3a0aa2e64629f9597af6c171915d48bf", // Replace "APIKEY" with your own API key; otherwise, your HTTP request will not work
    ...
    updateUISuccess: function(response) {
      ...
      listWidgets.updateActivityList();
    },
    updateUIFailure: () => {
...
document.querySelector('.forecast-button').addEventListener('click', function(e) {
  ...
  fetch(listWidgets.url + location + '&appid=' + listWidgets.apiKey).then(function(response) {
    return(response.json());
  }).then(function(response) {
    listWidgets.updateUISuccess(response);
  }).catch(function(response) {
    listWidgets.updateUIFailure();
  });
}, false);
...
document.querySelectorAll('.options div').forEach(function(el) {
  el.addEventListener('click', function(event) {
    listWidgets.updateActivityList(event);
